...
function ProjectInfoForm(props){
	...
	const formdef=[
		...
		
		{
			...
			validfunc:'expect($this).to.not.be.empty()'
		},{
			name:'othertags',
			type:'text',
			displayif:{field:'category',test:'includes',value:'other'},
			displayfunc:"expect($category).to.include('other')",
			...
			validfunc:'expect($this,"Please enter a valid email").to.match(emailregex)'
		},{
			name:'peopleinvolved',
			type:'typeahead',
			...
			validfunc:"expect($this,'Finish date must be later than start date').to.be.at.least($startdate)"
		},{
			name:'candisplay',
			type:'radio',
			options:{'Yes':'Yes',
