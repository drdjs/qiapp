...
function BaseCheckboxComponent({Component,setValue,numCols,disabled,label,name,required,helptext,options,optionsfrom,value=[],valid,validmessage,display}){
        ...
        return (
            <FormRow {...{valid,validmessage,required,display,label,helptext}}>	
...
function DropdownComponent({setValue,value=[],search,disabled,optionsfrom,label,placeholder,formname,name,required,helptext,options,multiple,allowNew,valid,validmessage,display}){
        //console.log(`refreshing ${name} as ${value} (display:${display})`)
        return(
            <FormRow {...{valid,validmessage,display,label,helptext,required}}>
...
function DatePickerComponent({label,disabled,name,required,helptext,value="",dispatch,valid,validmessage,display,setValue}){
...
function InputComponent({componentClass,disabled,label,required,name,type,placeholder,helptext="",value="",setValue,valid,validmessage,display}){
        return (
            <FormRow {...{valid,validmessage,display,label,helptext,required}}>
                <Form.Input
                    name={name}
...
function TextAreaComponent({componentClass,disabled,label,name,type,required,placeholder,helptext="",value="",setValue,valid,validmessage,display}){
        //console.log(`refreshing ${name} as ${value} (display:${display})`)
        return (
            <FormRow {...{valid,validmessage,display,label,helptext,required}}>
                <Form.TextArea
...
function FormRow({valid,validmessage,display,label,helptext,required,children}){
            console.log(`valid state:${label} ${validmessage}`)
            return (
                ...
                <Table.Cell width={8}><Form.Field required={required} error={valid===false}>
                    ...
                    {valid ? null:(<Message error visible>{validmessage}</Message>)}
                    </Table.Cell>
                <Table.Cell width={8}>
