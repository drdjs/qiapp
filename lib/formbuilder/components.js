...
import {useSetSingleValue,useSetArrayValue} from './redux'


function RadioComponent(props){
        const setValue=useSetSingleValue(props.formname,props.name)
        const value=useData(props.formname,props.name)
        return(<BaseCheckboxComponent
                    Component={Form.Radio}
                    setValue={(val,isSet)=>(isSet?setValue(val):null)}
                    numCols={1}
...
function CheckboxComponent(props){
        const setValue=useSetArrayValue(props.formname,props.name)
        const value=useData(props.formname,props.name)
        return(<BaseCheckboxComponent
...
function DropdownComponent({search,disabled,optionsfrom,label,placeholder,formname,name,required,helptext,options,multiple,allowNew,valid,errors,display}){
...
function DatePickerComponent({label,disabled,name,required,helptext,dispatch,valid,errors,display}){
...
function InputComponent({componentClass,disabled,label,required,name,type,placeholder,helptext="",valid,errors,display}){
        const setValue=useSetSingleValue(props.formname,props.name)
        const value=useData(props.formname,props.name)
        return (
            <FormRow {...{valid,errors,display,label,helptext,required}}>
...
function TextAreaComponent({componentClass,disabled,label,name,type,required,placeholder,helptext="",valid,errors,display}){
        //console.log(`refreshing ${name} as ${value} (display:${display})`)
        const setValue=useSetSingleValue(props.formname,props.name)
        const value=useData(props.formname,props.name)
        return (
